...
import com.emo.emomediaplayerpro.model.data_layer.content_providers.GenreLoader;
...
public class ListGenreFragment extends Fragment implements GenreLoader.MediaProviderEventsListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        genreRecyclerAdapter = new GenreRecyclerAdapter(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //Toast.makeText(getContext(),nasheedList.get( (int)v.getTag() ).getGenreName(),Toast.LENGTH_SHORT ).show();
                frag_NavigationManager.startListTracksFragment(null,null,null,null, nasheedList.get((int)v.getTag()).getId());
            }
        });
    }
    ...
    @Override
    public void onListCreated(List<Nasheed> pTrackList) {
        this.nasheedList = pTrackList;
    ...
    private static class GenreRecyclerAdapter extends RecyclerView.Adapter<ListGenreFragment.GenreRecyclerAdapter.GenreHolder> {
        ...
        public static class GenreHolder extends RecyclerView.ViewHolder {
            ...
            public void bind(Nasheed nasheed, int position) {
                this.itemView.setTag(position); /* shoaib: to get clicked item position */
                tv_genreName.setText(nasheed.getGenreName());
                tv_genreTotalTracks.setText(String.valueOf(nasheed.getCountTracks())+" Track(s)");
        ...
        private List<Nasheed> adapterNasheedList;
        ...
        public void updateFoldersList(List<Nasheed> artistsList) {
            this.adapterNasheedList = artistsList;
        ...
        public void onBindViewHolder(ListGenreFragment.GenreRecyclerAdapter.GenreHolder holder, int position) {
            holder.bind(this.adapterNasheedList.get(position), position);
        }

        @Override
        public int getItemCount() {
            if (this.adapterNasheedList != null)
                return this.adapterNasheedList.size();
            return 0;
        }
    }
}
