...
import com.emo.emomediaplayerpro.view_ui.viewmodel.ArtFragViewModel;
...
public class AlbumArtFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        if (albumID != -1)
        {
            viewModel.getTrackAlbum(albumID).observeForever(new Observer<List<Collection>>() {
                @Override
                public void onChanged(@Nullable List<Collection> collectionList)
                {
                    if (collectionList.size() == 0)
                        return;
                    String artUri = collectionList.get(0).getAlbumArtURI();
                             ...
                             Bundle savedInstanceState)
    {
        ...
        imgv_AlbumArt = layout.findViewById(R.id.imgV_SliderAlbumArt);
        imgv_background_blurred = layout.findViewById(R.id.imgV_back_blurred);
        setImages();
        return layout;
    }
